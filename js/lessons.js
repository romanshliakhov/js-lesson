...
// // Создаём обещание внутри переменной req. Когда мы создаем обещаение, мы предполагаем, что оно может завершиться как положительно. так и отрицательно. Но в данный момент мы не знаем как оно завершится. У нас есть определенный промежуток времени до того как мы получим результат. Внутри Promise есть два аргумента: resolve и reject. По факту это аргументы, вместо которых будут подставляться функции. Если всё прошло правильно. то будем вызывать функцию resolve. Если что-то пошло не так, то будем вызывать reject. 
// const req = new Promise((resolve, reject) => { 
//     // Имитация синхронного кода
//     setTimeout(() => {
//         console.log('Идет подготовка данных...'); // Предполагаем что сервер вернул такое сообщение
    
//         const product = {
//             name: 'TV',
//             price: 2000
//         } // Вернул нам такие данные. И мы предполагаем, раз мы их получили, то вызываем resolve
    
//         resolve(product); // Говорит всё окей, обещание выполнилось
//     }, 2000); // И это все произойдет через 2 секунды
// });
// // Для того, чтобы обработать положительный результат есть метод then. ОН внутри себя принимает как раз тот аргумент с функцией. которая называется resolve
// req.then((product) => {
...
//         if (!blockObj.id) throw new SyntaxError(`В данних под номером ${i + 1} нет id`);
...
// const err = new SyntaxError('lorem');
